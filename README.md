...
**LLM 內部執行三階段**（前端以即時進度條呈現）：

| 階段 | 說明 |
|------|------|
| ① SERP 融合（90%） | 將 SERP Skill 找出的 Content Gap 注入 Prompt，規劃獨家段落 |
| ② YMYL 合規（95%） | 套用 RAG 撈出的合規段落，強制加入免責聲明與禁用語過濾 |
| ③ 建議書生成（100%） | Gemini 2.5 Flash 輸出完整 JSON 格式規劃建議書 |
